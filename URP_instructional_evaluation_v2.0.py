...
def getInfo():
    ...
    html = response.read().decode('gb2312', 'ignore')		##注意第二个参数ignore，忽略非法字符，不加可能报错
    sub = "<img name="
    ...
    for i in range(0, count):
        ...
        m = tmp.find('#@')
        n = tmp.find('"')
        o = tmp.find('2015-2016-2-xs')		##第几学年第几学期
        bpr1 = tmp[m+2:m+8]
...
def postPj(br, pr, bm, pm):
    ...
    predata = {}
    predata['wjbm'] = '0000000024'	##抓包获得此处的值
    predata['bpr'] = br         ##课序号
    predata['pgnr'] = pr        ##课程号
    predata['oper'] = 'wjShow'
    predata['wjmc'] = '2015-2016-2-xs'
    predata['bprm'] = bm        ##教师
